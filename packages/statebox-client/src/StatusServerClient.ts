...
import {ChangeListener, IConfig, IJob, IMonitor, MessageListener, StatusListener} from "./Interfaces";
...
export class StatusServerClient {
    ...
    setTrackingIds(monitorIds: string[], jobIds: string[] = []) {
    ...
    public clearJob = async (monitorId: string, job: IJob) => {
        ...
        this.connection.send(
            JSON.stringify({ action: "remove-job", monitorId, listenerId: await this.getId(), jobId: job.id }),
        );
    };
    ...
    private processMessage(message: any) {
        ...
        switch (message.event) {
            case "job-update":
                index = this.monitors.findIndex((el) => el.id === message.monitorId);
                monitor = this.monitors[index];
                index = monitor.jobs.findIndex((el) => el.id === message.job.id);

                monitor.jobs[index] = { ...monitor.jobs[index], ...message.job };
            ...
            case "monitor-new":
                ...
                break;
            case "monitor-remove":
                index = this.monitors.findIndex((el) => el.id === message.monitor.id);
                if (index !== -1) {
        ...
        for (const currMonitor of this.monitors) {
            for (const jobIndex in currMonitor.jobs) {
                if (currMonitor.jobs[jobIndex].logs.length >= this.maxLogSize) {
                    currMonitor.jobs[jobIndex].logs = currMonitor.jobs[jobIndex].logs.slice(
