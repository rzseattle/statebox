  ...
  public process(
    ...
    switch (message.event) {
      case STATEBOX_EVENTS.JOB_UPDATED: {
        ...
        if (monitor == undefined) {
          ...
          monitor = this.state.monitors[this.state.monitors.length - 1];
        }
        index = monitor.jobs.findIndex((el) => el.jobId === message.jobId);
      ...
      case STATEBOX_EVENTS.JOB_DELETED: {
        ...
        if (monitor) {
          if (this.keepDone > 0) {
            const doneJobs = monitor.jobs.filter((job) => job.done).length;
    ...
    for (const currMonitor of this.state.monitors) {
      for (const jobIndex in currMonitor.jobs) {
        if (currMonitor.jobs[jobIndex].logsPart.length >= this.maxLogSize) {
          currMonitor.jobs[jobIndex].logsPart = currMonitor.jobs[
            jobIndex
          ].logsPart.slice(
            currMonitor.jobs[jobIndex].logsPart.length - this.maxLogSize
          );
        }
      }
    }
